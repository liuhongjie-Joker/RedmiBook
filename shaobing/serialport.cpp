...
bool SerialPort::get_Mode(int &mode, int &buff_four, int &shoot_speed, int &my_color,
                          double &yaw_angel, double &picth_angel, double &yaw_angel_speed,
                          double &picth_angel_speed)
{
    int bytes;
...
//        cout << "缓冲区为空" << endl;
    ...
    if (rdata[0] == 0xA5 && Verify_CRC8_Check_Sum(rdata, 3))
    {
        ...
        mode  = (int)rdata[1];
//        printf("-----------------------------------------------接收到的指令:%d\r\n", mode);
//        buff_four  = (int)rdata[3];
// //        printf("Is in sentry mode ? :%d\r\n", buff_four);
//        shoot_speed  = (int)rdata[4];
// //        printf("Is in base mode ? :%d\r\n", shoot_speed);
//        my_color = (int)rdata[5];
// //        printf("Is in my_color ? :%d\r\n", my_color);
        charTofloat cf;
        for(int i = 0; i < 4; i++)
        {
            cf.value[i] = rdata[3+i];
        }
        yaw_angel = (double)cf.fvalue;
        for(int i = 0; i < 4; i++)
        {
            cf.value[i] = rdata[7+i];
        }
        picth_angel = (double)cf.fvalue;
        for(int i = 0; i < 4; i++)
        {
            cf.value[i] = rdata[11+i];
        }
        yaw_angel_speed = (double)cf.fvalue;
        for(int i = 0; i < 4; i++)
        {
            cf.value[i] = rdata[15+i];
        }
        picth_angel_speed = (double)cf.fvalue;
    }
    return true;
}
...
void SerialPort::TransformData(const VisionData &data)
{
    ...
    //Tdata[15] = data.buff_change_four;
